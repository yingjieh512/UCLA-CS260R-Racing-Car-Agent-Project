...
from agents.example_agent.agent import Policy
from racing_maps import set_racing_map
from racing_maps import RACING_MAPS
class ModelptOpponent:
    def __init__(self, model_path):
        self.policy = Policy()

    def __call__(self, obs, agent_id):
        return self.policy(obs)
eg_agent_ob              = ModelptOpponent("/content/my_mini_project/mini_project/agents/example_agent/model.pt")
baseline_agent_ob        = ModelptOpponent("/content/my_mini_project/mini_project/agents/example_agent/modelbaseline.pt")
original_agent_ob        = ModelptOpponent("/content/my_mini_project/mini_project/agents/example_agent/modeloriginal.pt")
originaladdoppo_agent_ob = ModelptOpponent("/content/my_mini_project/mini_project/agents/example_agent/orginialAddoppo.pt")
largenstepsoppo_agent_ob = ModelptOpponent("/content/my_mini_project/mini_project/agents/example_agent/modellargenstepsoppo.pt")
randomstrongoppo_agent_ob= ModelptOpponent("/content/my_mini_project/mini_project/agents/example_agent/randstrongoppo.pt")
best_agent_ob            = ModelptOpponent("/content/my_mini_project/mini_project/agents/example_agent/agentbest.pt")
...
def slow_opponent(obs, agent_id):
    return np.array([0.005, 0.004], dtype=np.float32)

def still_opponent(obs, agent_id):
    return np.array([0.0, 0.0], dtype=np.float32)

def eg_agent(obs,agent_id):
    return eg_agent_ob(obs,agent_id)

def baseline_agent(obs,agent_id):
    return baseline_agent_ob(obs,agent_id)

def original_agent(obs,agent_id):
    return original_agent_ob(obs,agent_id)

def originaladdoppo_agent(obs,agent_id):
    return originaladdoppo_agent_ob(obs,agent_id)

def largenstepsoppo_agent(obs,agent_id):
    return largenstepsoppo_agent_ob(obs,agent_id)

def randomstrongoppo_agent(obs,agent_id):
    return randomstrongoppo_agent_ob(obs,agent_id)

def best_agent(obs,agent_id):
    return best_agent_ob(obs,agent_id)
...
OPPONENT_POLICIES = {
    ...
    "original_addoppo": originaladdoppo_agent,
    "largenstep_oppo":largenstepsoppo_agent,
    "randomstrong_oppo": randomstrongoppo_agent,
    "BASELINE":best_agent,         
}
...
class RacingEnv(gymnasium.Env):
    ...
    def step(self, action):
        ...
        speed = ego_info.get("speed_km_h", 0.0)
        velocity = ego_info.get("velocity", 0.0)
        steering = ego_info.get("steering", 0.0)
        acceleration = ego_info.get("acceleration", 0.0)

        progress = ego_info.get("progress", 0.0)
        overtake = ego_info.get("overtake_vehicle_num", 0)

        navigation_forward = ego_info.get("navigation_forward", False)
        navigation_left = ego_info.get("navigation_left", False)
        navigation_right = ego_info.get("navigation_right", False)

        crash = ego_info.get("crash", False)
        out_of_road = ego_info.get("out_of_road", False)
        arrive_dest = ego_info.get("arrive_dest", False)
        max_step = ego_info.get("max_step", False)
        ego_reward += 5*overtake
        # ego_reward +=0.001*speed
...
def make_racing_env(rank=0, num_agents=2, opponent_policy="aggressive", extra_config=None):
    def _init():
        maps = ["circuit","hairpin","Oval Server Map","Serpentine Server Map"]
        restore = set_racing_map(maps[rank % len(maps)])
        env = RacingEnv(
            num_agents=2,
